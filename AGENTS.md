...
| Module lifecycle / registration mechanism itself | For an embedded `my`-side module, `EmbeddedModuleInterface` is canonical here. `rc-core`'s own `ModuleInterface`/`ModuleRegistry` mechanism is a **separate, real** contract used in production by RC-Catalog (`www`-side) — it is not dead code and was restored after a mistaken removal broke production (2026-09-19). See `rc-core/docs/MODULE-DEVELOPMENT.md` and `rc-core/docs/ARCHITECTURE-OPEN-QUESTIONS.md` #1 (open) |
